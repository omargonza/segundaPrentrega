...
routerUsers.post("/register/create", passport.authenticate('register'), userController.save)

//para ingresar el usuario
routerUsers.get("/login", userController.login)
routerUsers.post("/login/into", passport.authenticate('login'), userController.access)

//para el ingreso de un usuario a traves de github
routerUsers.get("/github", passport.authenticate('github', { scope: ['user:email'] }))
routerUsers.get("/githubcallback", passport.authenticate('github'), userController.githubLogin)
